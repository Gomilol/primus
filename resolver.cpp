...
void Resolver::ResolveStand(AimPlayer* data, LagRecord* record, Player* player) {
	...
	// if we missed more then one shot, lets force LBY


	// this is our first case, without shooting, so we need to force antifreestand as a means to logic
	else {
		AntiFreestand(record);
		resolver_mode = XOR("Anti Freestanding\n");
	}
		break;

	case 1: // lets go for antifreestand one more time yea?
		if (data->m_moved) {
			float diff = math::NormalizedAngle(record->m_body - move->m_body);
			float delta = record->m_anim_time - move->m_anim_time;
		...
		break;
	case 2: // after our 2nd shot, lets force them to -90 degrees (sideways)
		if (data->m_moved) {
			float diff = math::NormalizedAngle(record->m_body - move->m_body);
			float delta = record->m_anim_time - move->m_anim_time;
			...
			record->m_eye_angles.y = move->m_body + -90.f;
			resolver_mode = XOR("Brute (-90)\n");
		}
		break;
		// 3rd shot is gonna be the same but opposite directions
	case 3:
		if (data->m_moved) {
			float diff = math::NormalizedAngle(record->m_body - move->m_body);
			float delta = record->m_anim_time - move->m_anim_time;
		...
		break;
		// yeah lets try something to stop
	case 4:
		if (data->m_moved) {
			float diff = math::NormalizedAngle(record->m_body - move->m_body);
			float delta = record->m_anim_time - move->m_anim_time;
			...
			if (act != 980) {
				...
				resolver_mode = XOR("Brute (body + 110)\n");
	...
	case 5:
		if (data->m_moved) {
			...
			AntiFreestand(record);
			resolver_mode = XOR("Anti Freestand\n");
		}
		break;
	...
	case 7:
		if (data->m_moved) {
			...
			if (act != 980) {
				record->m_eye_angles.y = move->m_body + 70.f;
				resolver_mode = XOR("Brute (70)\n");
	...
	if (data->m_stand_index > 3) {
		record->m_eye_angles.y = move->m_body; // b1g resolvens
	}
	// if we keep missing, lets just keep moving there yaw so we STOP FUCKING DUMPING.
	if (data->m_stand_index > 6 && act != 980) {
		// lets just hope they switched ang after move.
		record->m_eye_angles.y = move->m_body + 180.f;
	}
	// if all is failing lets go back to fucking antifreestand
	if (data->m_missed_shots > 9) {
		AntiFreestand(record);
	}
	// and finally, lets start with anti-freestand.
	else {
		AntiFreestand(record);
	}*/
}
